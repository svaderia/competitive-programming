...
struct suffix_array {
    ...
    void build(const string &inp, bool build_rmq = false) {
        s = inp;
        s.pb('$'); // assumes that the '$' is not part of the string and largest.
...
void solve(){
    ...
    dbg(SA.lcp);
    rep(i, sz(con)){
        int mn = min(SA.suffix[i], SA.suffix[i + 1]);
        int mx = max(SA.suffix[i], SA.suffix[i + 1]);
        if(SA.lcp[i] > ans && mn < sz(s) && mx > sz(s)){
            ans = SA.lcp[i];
            which = mn;
        }
    }
