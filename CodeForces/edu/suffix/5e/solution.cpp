...
void solve() {
    ...
    rep(i, n) {
        int curr = SA.lcp[i] * (right[i] - left[i]);
        if (ans < curr) {
            ans = curr;
            idx = i;
        }
    ...
    cout << ans << endl;
    cout << SA.lcp[idx] << endl;

    int ka = SA.suffix[idx];
    dbg(ka);

    rep(i, SA.lcp[idx]) {
        cout << int(s1[ka + i] - '0') << spc;
    }
    cout << endl;
}
