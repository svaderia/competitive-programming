...
bool p(lli h, lli r, lli mid, lli W){
    return (double) mid >= (W - h) / (double)r;
}


lli bin_search(lli h, lli r, lli lo, lli hi, lli W){
    ...
    if(W > L){
        rep(i, N){
            if( (double)v[i].first >= (W - ah[i])/ (double)ar[i]){
                ans = bin_search(ah[i-1], ar[i-1], days[i-1], days[i], W);
                if(ans == -1)
                    ans = days[i];
                break;
            }
