...
node merge(node left, node right){
    node id;
    id.allsum = left.allsum + right.allsum;
    id.leftsum = max({left.leftsum, left.allsum + right.leftsum});
    id.rightsum = max({right.rightsum, right.allsum + left.rightsum});
    id.maxsum = max({id.leftsum, id.rightsum, left.rightsum + right.leftsum, left.maxsum, right.maxsum});
    return id;
}
...
node query(int x, int y, int id = 1, int l = 0, int r = N){
    ...
    node left_node = query(x,  y, left, l, mid);
    node right_node = query(x, y, right, mid, r);
