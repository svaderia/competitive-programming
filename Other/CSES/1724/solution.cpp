...
struct Matrix {
    int n;
    vvi a;
    Matrix(int nn) : n(nn), a(n, vi(n, INF)) {}
...
void solve(){
    ...
    rep(i, m){
        ...
        cin >> a >> b >> c;
        a--, b--;
        fs.a[a][b] = min(fs.a[a][b], c);
    }

    Matrix prod = expo_power(fs, k);

    if(prod.a[0][n - 1] == INF) prod.a[0][n - 1] = -1;
    cout << prod.a[0][n - 1] << endl;
