...
void solve(){
    ...
    while(!pq.empty()){
        ...
        for(auto p : g[root]){
            ...
            }else if(dp[child] == d + w){
                mn[child] = min(mn[child], 1 + mn[root]);
                mx[child] = max(mx[child], 1 + mx[root]);
                tot[child] += tot[root];
            }
        }
    }
    ...
    int end = n - 1;
    cout << dp[end] << spc << tot[end] << spc << mn[end] << spc << mx[end] << endl;
